...
public class OnlineHotelConstructor : NetworkBehaviour
{
    ...
    OnlineSeed hotelSeed = new OnlineSeed(); //Creates new seed object
    ...
    void Start()
    {
        if (isServer)
        {
            seed = generateSeed(); //Generates new seed
        }
    ...
    //Method to disable most of the hotel for better performance
    private void disableRooms()
    {
        //Get all of the room, hallway, and stairway gameobjects and put them into their respective arrays
        GameObject[] rooms = GameObject.FindGameObjectsWithTag("room");
    ...
    {
        //Create a random int between 10,000 and 2,147,483,647
        int rand = UnityEngine.Random.Range(10000, 2147483647);
        return rand;
    }
}
